	...
	var d = {
		...
		"./node_modules/chaimu/dist/index.js": (d, f, p) => {
			...
			class AudioPlayer extends BasePlayer {
				...
				syncPlay() {
					return h.log("[AudioPlayer] sync play called"), this.audio.play().catch(this.audioErrorHandle), this;
				}
				async play() {
					return h.log("[AudioPlayer] play called"), await this.audio.play().catch(this.audioErrorHandle), this;
				}
				async pause() {
					return h.log("[AudioPlayer] pause called"), this.audio.pause(), this;
				}
				set src(d) {
					if (this._src = d, !d) {
						this.clear();
			...
			class ChaimuPlayer extends BasePlayer {
				...
				createAudioElement() {
					if (!this.chaimu.audioContext) throw Error("No audio context available");
					if (!this.blobUrl) throw Error("No blob URL available. Did you call fetchAudio()?");
				...
				async reopenCtx() {
					...
					try {
						...
						console.warn("Failed to close audio context:", d);
					}
					return this;
				}
				async clear() {
					if (!this.chaimu.audioContext) throw Error("No audio context available");
					if (this.isClearing) return this;
					h.log("clear audio context"), this.isClearing = !0;
					try {
						...
						let d = this.volume;
						return await this.reopenCtx(), this.chaimu.audioContext = initAudioContext(), this.initAudioBooster(), this.volume = d, this;
					} finally {
						this.isClearing = !1;
					}
				}
				...
				async pause() {
					if (!this.chaimu.audioContext) throw Error("No audio context available");
					return this.chaimu.audioContext.state === "running" ? (await this.chaimu.audioContext.suspend(), this.audioElement && this.audioElement.pause(), this) : this;
				}
				async play() {
					if (!this.chaimu.audioContext) throw Error("No audio context available");
			...
			p.a(d, async (d, m) => {
				try {
					...
					class VideoHandler {
						...
						subtitles = [];
						constructor(d, f, p) {
							x.A.log("[VideoHandler] add video:", d, "container:", f, this), this.video = d, this.container = f, this.site = p, this.abortController = new AbortController(), this.actionsAbortController = new AbortController(), this.uiManager = new U.o({
								root: this.container,
